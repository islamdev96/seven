...
import 'package:seven/view/screens/checkout/widget/cancel_dialog.dart';
...
class MyInAppBrowser extends InAppBrowser {
  ...
  void _pageRedirect(String url) {
    if (_canRedirect) {
      ...
      if (_isSuccess) {
        String _token = url.replaceAll(
            '${AppConstants.BASE_URL}${RouteHelper.orderSuccessful}/success?token=',
            '');
        print('token is: $_token');
        if (_token != null) {
          String _decodeValue =
              utf8.decode(base64Url.decode(_token.replaceAll(' ', '+')));
          String _paymentMethod =
              _decodeValue.substring(0, _decodeValue.indexOf('&&'));
          String _transactionReference = _decodeValue.substring(
              _decodeValue.indexOf('&&') + '&&'.length, _decodeValue.length);
        ...
        Navigator.pushReplacementNamed(context,
            '${RouteHelper.orderSuccessful}/${orderModel!.id}/payment-cancel');
      }
    }
  }
